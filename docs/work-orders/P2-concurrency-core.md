...
>
> **이름 통일 (2026-08-13 리뷰 반영):** 지시서 제목·파일명(`P2-concurrency-core.md`),
> 이슈 #25, 위키 ADR 로드맵 모두 **"동시성 코어"**로 맞춘다. 검색·추적 시 "스케줄 루프"/"배치 루프"로 혼용하지 않는다.
...
```
 ...
 HTTP req → │ axum handlers (tokio, async)                 │
            ...
            └───────────────┬──────────────────────────────┘
                            ...
                            ▼
            ┌──────────────────────────────────────────────┐
 HTTP res ← │ SSE 인코더 → 클라이언트 (choices[].delta.content)│
            └──────────────────────────────────────────────┘
```

**결과 역방향 흐름:** GPU 워커에서 생성된 토큰은 슬롯별 스트림(oneshot/mpsc)을 타고
스케줄러 → 해당 HTTP 핸들러 → SSE로 흘러나간다. 각 요청은 자기 슬롯의 토큰 스트림만 구독한다.
(위 다이어그램의 아래쪽 역방향 화살표가 그 경로다.)
...
> **먼저 (체크리스트 맨 앞):** `docs/bench/` 디렉토리가 없다면 생성한다.
> P2는 `docs/bench/p2.md`, P3는 `docs/bench/p3.md`에 벤치 결과를 남기므로, 이 디렉토리가 없으면 구현자가 헛돈다.
