...
- **P3가 닫힌 뒤에도 남은 것 (2026-08-13, #26 done 유지):**
  - 생산 다턴은 여전히 `cache_n=0`. `finish_slot`이 매번 `clear_seq`. → **P5 / #28**
  - §4.5 rocprof·gfx906 커널 튜닝은 선택이라 안 했다. Rust가 아니라 HIP C++. → **P6 / #29**
- P4는 선택. P6 프로파일까지 **모두** 만족할 때만 착수한다 (P4 지시서 gate).
